...
public class GrpcAccountService extends AccountImplBase {
    ...
    public void restoreAccount(RestoreAccountRequest req, StreamObserver<RestoreAccountReply> responseObserver) {
        try {
            coreApi.restoreAccount(Attachment.fromProto(req.getAttachment()));
    ...
    public void backupAccount(BackupAccountRequest req, StreamObserver<BackupAccountReply> responseObserver) {
        try {
          Attachment attachment = coreApi.backupAccount();          
          responseObserver.onNext(BackupAccountReply.newBuilder()
                          .setAttachment(attachment.toProtoMessage())
                          .build());
          responseObserver.onCompleted();
        } catch (Throwable cause) {
            exceptionHandler.handleException(log, cause, responseObserver);
    ...
    final Optional<ServerInterceptor> rateMeteringInterceptor() {
        return getCustomRateMeteringInterceptor(coreApi.getConfig().appDataDir, this.getClass())
                .or(() -> Optional.of(CallRateMeteringInterceptor.valueOf(
                        new HashMap<>() {{
                        	put(getAccountExistsMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                        	put(getBackupAccountMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                        	put(getChangePasswordMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                        	put(getCloseAccountMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                        	put(getCreateAccountMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                        	put(getDeleteAccountMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                            put(getIsAccountOpenMethod().getFullMethodName(), new GrpcCallRateMeter(5, SECONDS));
                            put(getOpenAccountMethod().getFullMethodName(), new GrpcCallRateMeter(5, MINUTES));
