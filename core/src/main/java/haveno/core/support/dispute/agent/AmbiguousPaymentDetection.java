...
public class AmbiguousPaymentDetection {
    ...
    public AmbiguousPaymentDetection(DisputeManager<? extends DisputeList<Dispute>> disputeManager) {
        ...
        disputeManager.getDisputesAsObservableList().addListener((ListChangeListener<Dispute>) c -> {
            c.next();
            if (c.wasAdded()) detectAmbiguousPayments();
        });
    }
    ...
    // key by the payment details of both accounts and the trade amount, since payment account ids and
    // payload salts are not stable identities for the same external account; null if unavailable
    private static String getKey(Dispute dispute) {
        try {
            ...
            return Utilities.encodeToHex(Hash.getSha256Hash(buyerPayload.getPaymentDetails())) + "|" +
                    Utilities.encodeToHex(Hash.getSha256Hash(sellerPayload.getPaymentDetails())) + "|" +
