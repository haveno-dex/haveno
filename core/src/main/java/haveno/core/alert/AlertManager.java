...
public class AlertManager {
    ...
    private void signAndAddSignatureToAlertMessage(Alert alert) {
        alert.setOwnerPubKey(keyRing.getSignatureKeyPair().getPublic()); // set before signing so the signature binds the owner key
        String signaturePayload = alert.getSignaturePayloadAsHex();
        alert.setSignatureAsBase64(alertSigningKey.signMessage(signaturePayload));
    }
