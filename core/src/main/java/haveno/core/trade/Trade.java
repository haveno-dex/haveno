...
public abstract class Trade extends XmrWalletBase implements Tradable, Model {
    ...
    private static final long IDLE_SYNC_PERIOD_MS = Config.baseCurrencyNetwork().isTestnet() ? 45000 : 1680000; // 28 minutes (monero's default connection timeout is 30 minutes on a local connection, so beyond this the wallets will disconnect)
    private static final long MAX_REPROCESS_DELAY_SECONDS = 7200; // max delay to reprocess messages (once per 2 hours)
    private static final long CHECK_POOL_PERIOD_MS = Config.baseCurrencyNetwork().isTestnet() ? 30000 : 180000; // throttle fetching from tx pool since invalidation is unlikely
