...
public final class MpesaAccount extends CountryBasedPaymentAccount {
    ...
    // M-Pesa operates natively via local telecom partnerships in these countries, each with its own local currency.
    // The currency is derived from the selected country, so each account holds exactly the country's currency.
    // The DR Congo is dollarized and also settles in USD, but is intentionally kept to CDF to preserve that 1:1 mapping.
    ...
    public void setCountry(Country country) {
        ...
        setSingleTradeCurrency(CurrencyUtil.getCurrencyByCountryCode(country.code)); // each M-Pesa market has its own currency
    }
}
