    ...
    protected void run() {
        try {
            ...
            if (trade.getMakerDepositTxId() != null)
                throw new RuntimeException("Maker deposit tx already set for trade " + trade.getId() + ", this should not happen"); // TODO: ignore and nack bad requests to not show on client
            ...
            if (takerGoesFirst) {
                ...
                if (message.getDepositTxId() == null)
                    throw new RuntimeException("Taker must prove deposit tx before maker deposits");
