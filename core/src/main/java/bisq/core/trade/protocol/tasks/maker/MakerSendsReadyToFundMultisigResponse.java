...
public class MakerSendsReadyToFundMultisigResponse extends TradeTask {
    ...
    protected void run() {
        try {
            ...
            // TODO (woodser): close trade when taker deposits to multisig or when taker first pays trade fee?  rename trade state?
            // TODO (woodser): does reserving trade prevent another trader from paying trade fee?
            trade.setState(Trade.State.MAKER_SENT_PUBLISH_DEPOSIT_TX_REQUEST);  // TODO (woodser): this removes offer from order book.  when to remove offer from order book?
