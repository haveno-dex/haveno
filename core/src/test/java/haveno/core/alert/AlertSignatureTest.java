...
public class AlertSignatureTest {
    ...
    // The signature must also bind the storage owner key, so a captured alert cannot be cloned under
    // another owner key which the devs cannot remove or supersede.
    @Test
    public void signatureCoversOwnerPubKey() throws SignatureException {
        ECKey devKey = new ECKey();
        Alert genuine = new Alert("please update", true, false, "1.2.3");
        genuine.setOwnerPubKey(Sig.generateKeyPair().getPublic());
        String signature = devKey.signMessage(genuine.getSignaturePayloadAsHex());

        assertDoesNotThrow(() -> devKey.verifyMessage(genuine.getSignaturePayloadAsHex(), signature));

        // same alert re-owned by another storage key -> captured signature must not verify
        Alert reOwned = new Alert("please update", true, false, "1.2.3");
        reOwned.setOwnerPubKey(Sig.generateKeyPair().getPublic());
        assertThrows(SignatureException.class,
                () -> devKey.verifyMessage(reOwned.getSignaturePayloadAsHex(), signature));
    }
}
