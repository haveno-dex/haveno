# Haveno API Beta Testing Guide

This guide explains how Haveno Api beta testers can quickly get a test harness running, watch a regtest trade simulation,
...
Clone the Haveno master branch into a project folder of your choice.  In this document, the root project folder is
called `api-beta-test`.
```
$ git clone https://github.com/haveno-network/haveno.git api-beta-test
```

## Running Api Test Harness

If your bitcoin-core binaries are in your system `PATH`, start bitcoind in regtest-mode, Haveno seednode and arbitration
node daemons, plus Bob & Alice daemons in a bash terminal with the following bash command:
```
$ ./haveno-apitest --apiPassword=xyz \
...
Alternatively, you can specify any or all of these haveno-apitest options in a properties file located in
`apitest/src/main/resources/apitest.properties`.

In this example, a beta tester uses the `apitest.properties` below, instead of `haveno-cli` options.
```
supportingApps=bitcoind,seednode,arbdaemon,alicedaemon,bobdaemon
apiPassword=xyz
shutdownAfterTests=false
...
You should see the test harness startup bitcoin-core and other Haveno daemons in your console, run a
...
The regtest trade simulation script `apitest/scripts/trade-simulation.sh` is a useful introduction to the Haveno Api.
...
```
$ ./haveno-cli  --help	(the –password option is not needed because there is no server request)
```

For help with a specific CLI command:
```
$ ./haveno-cli --password=xyz --help getbalance
OR
$ ./haveno-cli --password=xyz  getbalance --help
```
...
There is no need to secure your regtest Haveno wallet with an encryption password when running these examples,
but you should encrypt your mainnet wallet as you probably already do when using the Haveno UI to transact in
real BTC.  This section explains how to encrypt your Haveno wallet with the CLI, and unlock it before performing wallet
...
```
$ ./haveno-cli --password=xyz setwalletpassword --wallet-password=<wallet-password>
```
...
```
$ ./haveno-cli --password=xyz unlockwallet --wallet-password=<wallet-password> --timeout=<seconds>
```
...
```
$ ./haveno-cli --password=xyz lockwallet
```
...
```
$ ./haveno-cli --password=xyz --port=9999 getbalance --currency-code=btc
```

### Funding a Haveno Wallet

#### Receiving BTC

To receive BTC from an external wallet, find an unused BTC address (with a zero balance) to receive the BTC.
```
$ ./haveno-cli --password=xyz --port=9998 getfundingaddresses
```
You can check a block explorer for the status of a transaction, or you can check your Haveno BTC wallet address directly:
```
$ ./haveno-cli --password=xyz --port=9998 getaddressbalance --address=<btc-address>
```
...
Send BTC:
```
$ ./haveno-cli --password=xyz --port=9998 sendbtc --address=<btc-address> --amount=<btc-amount>
```
Send BTC with a withdrawal transaction fee of 20 sats/byte:
```
$ ./haveno-cli --password=xyz --port=9998 sendbtc --address=<btc-address> --amount=<btc-amount>  --tx-fee-rate=20
```

### Withdrawal Transaction Fees

If you have traded using the Haveno UI, you are probably aware of the default network bitcoin withdrawal transaction
...
```
$ ./haveno-cli --password=xyz unsettxfeerate
```
...
2.  Use the payment-method-id `F2F` found in the `getpaymentmethods` command output to create a blank payment account
    form:
    ```
    $ ./haveno-cli --password=xyz --port=9998 getpaymentacctform --payment-method-id=F2F
    ```
...
3.  Manually edit the json file, and use its path in the `createpaymentacct` command.
    ```
    $ ./haveno-cli --password=xyz --port=9998 createpaymentacct \
    ...
    ```
    $ ./haveno-cli --password=xyz --port=9998 getpaymentaccts
    ```
...
```
$ ./haveno-cli --password=xyz --port=9998 createoffer --help
```
...
and pay the Haveno maker fee in BTC.  Alice had already created an EUR face-to-face payment account with id
`f3c1ec8b-9761-458d-b13d-9039c6892413`, and used this `createoffer` command:
```
$ ./haveno-cli --password=xyz --port=9998 createoffer \
...
```
$ ./haveno-cli --password=xyz --port=9998 getmyoffers --direction=<BUY|SELL> --currency-code=<currency-code>
```

To look at a specific offer you created:
```
$ ./haveno-cli --password=xyz --port=9998 getmyoffer --offer-id=<offer-id>
```
...
```
$ ./haveno-cli --password=xyz --port=9998 getoffers --direction=<BUY|SELL> --currency-code=<currency-code>
```

To look at a specific, available offer you could take:
```
$ ./haveno-cli --password=xyz --port=9998 getoffer --offer-id=<offer-id>
```
...
```
$ ./haveno-cli --password=xyz --port=9998 canceloffer --offer-id=<offer-id>
```
The offer will be removed from other Haveno users' offer views, and paid transaction fees will be forfeited.
...
```
$ ./haveno-cli --password=xyz --port=9998 takeoffer \
...
The first step in the Haveno trade protocol is completed when a `takeoffer` command successfully creates a new trade from
the taken offer.  After the Haveno nodes prepare the trade, its status can be viewed with the `gettrade` command:
```
$ ./haveno-cli --password=xyz --port=9998 gettrade --trade-id=<trade-id>
```
...
option:
```
$ ./haveno-cli --password=xyz --port=9998 gettrade --trade-id=<trade-id> --show-contract=true
```
...
keepfunds                   Keep trade proceeds in their Haveno wallets.
    OR
...
because they inform the Haveno node that a trade’s state can be set to `CLOSED`.  Please close out your trades with one
or the other command.

Each of the CLI commands above takes one argument:  `--trade-id=<trade-id>`:
```
$ ./haveno-cli --password=xyz --port=9998 confirmpaymentstarted --trade-id=<trade-id>
$ ./haveno-cli --password=xyz --port=9999 confirmpaymentreceived --trade-id=<trade-id>
$ ./haveno-cli --password=xyz --port=9998 keepfunds --trade-id=<trade-id>
$ ./haveno-cli --password=xyz --port=9999 withdrawfunds --trade-id=<trade-id> --address=<btc-address> [--memo=<"memo">]
```
...
Once shutdown, all Haveno and bitcoin-core data files are left in the state they were in at shutdown time,
