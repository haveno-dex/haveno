...
dw_source() {
	echo "-> Trying to use wget to download Bitcoin and Monero binaries..."
...
dw_monero() {
	...
	tar -xzf monero-bins-haveno-${platform}.tar.gz -C .localnet/
	cp .localnet/monero-bins-haveno-${platform}/monero-wallet-rpc monero-wallet-rpc
...
dw_bitcoin() {
	...
	tar -xzf bitcoin-${btcversion}-${btc_platform}.tar.gz -C .localnet/
	cp .localnet/bitcoin-${btcversion}/bin/{bitcoin-cli,bitcoind} .localnet/
	rm bitcoin-${btcversion}-${btc_platform}.tar.gz
}


while true
do
	if [ ! -d .localnet/monero-bins-haveno-${platform} ]; then
		dw_monero
	fi

	if [ ! -d .localnet/bitcoin-${btcversion} ]; then
		dw_bitcoin
		mkdir .localnet/bitcoin-node
	fi
	exit
done
