...
@Slf4j
public class PaparaForm extends PaymentMethodForm {
    private final PaparaAccount paparaAccount;
    private final AccountNrValidator accountNrValidator;

    public PaparaForm(PaymentAccount paymentAccount,
    ...
    public void addFormForAddAccount() {
        ...
        holderNameInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paparaAccount.setHolderName(newValue);
            updateFromInputs();
        });
        ...
        accountNrInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paparaAccount.setAccountNr(newValue);
            updateFromInputs();
        });
        ...
        bankNameInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paparaAccount.setBankName(newValue);
            updateFromInputs();
        });

        TradeCurrency singleTradeCurrency = paparaAccount.getSingleTradeCurrency();
    ...
    protected void autoFillNameTextField() {
        setAccountNameWithString(paparaAccount.getAccountNr());
    }
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                && inputValidator.validate(paparaAccount.getHolderName()).isValid
                && accountNrValidator.validate(paparaAccount.getAccountNr()).isValid
                && inputValidator.validate(paparaAccount.getBankName()).isValid
                && paparaAccount.getTradeCurrencies().size() > 0);
    }
}
