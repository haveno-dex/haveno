...
@Slf4j
public class PayNowForm extends PaymentMethodForm {
    private final PayNowAccount paynowAccount;
    private final AccountNrValidator accountNrValidator;

    public PayNowForm(PaymentAccount paymentAccount,
    ...
    public void addFormForAddAccount() {
        ...
        holderNameInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paynowAccount.setHolderName(newValue);
            updateFromInputs();
        });
        ...
        accountNrInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paynowAccount.setAccountNr(newValue);
            updateFromInputs();
        });
        ...
        bankNameInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            paynowAccount.setBankName(newValue);
            updateFromInputs();
        });

        TradeCurrency singleTradeCurrency = paynowAccount.getSingleTradeCurrency();
    ...
    protected void autoFillNameTextField() {
        setAccountNameWithString(paynowAccount.getAccountNr());
    }
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                && inputValidator.validate(paynowAccount.getHolderName()).isValid
                && accountNrValidator.validate(paynowAccount.getAccountNr()).isValid
                && inputValidator.validate(paynowAccount.getBankName()).isValid
                && paynowAccount.getTradeCurrencies().size() > 0);
    }
}
