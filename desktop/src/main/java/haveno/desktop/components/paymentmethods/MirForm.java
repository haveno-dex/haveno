...
import haveno.core.payment.validation.MirValidator;
...
public class MirForm extends PaymentMethodForm {
    private final MirAccount account;
    private final MirValidator mirValidator = new MirValidator();

    public static int addFormForBuyer(GridPane gridPane, int gridRow,
                                      PaymentAccountPayload paymentAccountPayload) {
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((MirAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.cardNr"),
                ((MirAccountPayload) paymentAccountPayload).getAccountNr());
        return gridRow;
    }

    public MirForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                   InputValidator inputValidator, GridPane gridPane,
                   int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (MirAccount) paymentAccount;
    }
    ...
    public void addFormForAddAccount() {
        ...
        accountNrInputTextField.setValidator(mirValidator);
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                ...
                && mirValidator.validate(account.getAccountNr()).isValid);
    }
}
