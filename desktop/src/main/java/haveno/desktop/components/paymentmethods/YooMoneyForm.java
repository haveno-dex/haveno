...
public class YooMoneyForm extends PaymentMethodForm {
    private final YooMoneyAccount account;
    private final YooMoneyValidator yooMoneyValidator = new YooMoneyValidator();

    public static int addFormForBuyer(GridPane gridPane, int gridRow,
                                      PaymentAccountPayload paymentAccountPayload) {
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((YooMoneyAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.walletNr"),
                ((YooMoneyAccountPayload) paymentAccountPayload).getAccountNr());
        return gridRow;
    }

    public YooMoneyForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                   InputValidator inputValidator, GridPane gridPane,
                   int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (YooMoneyAccount) paymentAccount;
    }

    @Override
    public void addFormForAddAccount() {
        // this payment method is only for Russia/RUB
        account.setSingleTradeCurrency(account.getSupportedCurrencies().get(0));
        CountryUtil.findCountryByCode("RU").ifPresent(c -> account.setCountry(c));
        ...
        InputTextField accountNrInputTextField = FormBuilder.addInputTextField(gridPane, ++gridRow, Res.get("payment.walletNr"));
        accountNrInputTextField.setValidator(yooMoneyValidator);
        accountNrInputTextField.textProperty().addListener((ov, oldValue, newValue) -> {
            account.setAccountNr(newValue.trim());
            updateFromInputs();
        });
    ...
    protected void autoFillNameTextField() {
        setAccountNameWithString(account.getAccountNr());
    }
    ...
    public void addFormForEditAccount() {
        ...
        addCompactTopLabelTextField(gridPane, ++gridRow, Res.get("payment.walletNr"), account.getAccountNr())
                .second.setMouseTransparent(false);
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                ...
                && yooMoneyValidator.validate(account.getAccountNr()).isValid);
    }
}
