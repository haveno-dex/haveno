...
public class MpesaForm extends PaymentMethodForm {
    private final MpesaAccount account;
    ...
    public static int addFormForBuyer(GridPane gridPane, int gridRow,
        ...
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((MpesaAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.mobile"),
                ((MpesaAccountPayload) paymentAccountPayload).getMobileNr());
        return gridRow;
    }

    public MpesaForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                     InputValidator inputValidator, GridPane gridPane,
                     int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (MpesaAccount) paymentAccount;
    }
    ...
    public void addFormForAddAccount() {
        ...
        // the country determines the local M-Pesa currency (the DR Congo derives to its franc; it can also hold USD via the API)
