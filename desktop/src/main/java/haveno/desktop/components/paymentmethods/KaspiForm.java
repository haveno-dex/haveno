...
public class KaspiForm extends PaymentMethodForm {
    private final KaspiAccount account;
    private final KaspiValidator kaspiValidator = new KaspiValidator();

    public static int addFormForBuyer(GridPane gridPane, int gridRow,
                                      PaymentAccountPayload paymentAccountPayload) {
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((KaspiAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.kaspi.accountId"),
                ((KaspiAccountPayload) paymentAccountPayload).getAccountNr());
        return gridRow;
    }

    public KaspiForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                   InputValidator inputValidator, GridPane gridPane,
                   int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (KaspiAccount) paymentAccount;
    }

    @Override
    public void addFormForAddAccount() {
        // this payment method is only for Kazakhstan/KZT
        account.setSingleTradeCurrency(account.getSupportedCurrencies().get(0));
        CountryUtil.findCountryByCode("KZ").ifPresent(c -> account.setCountry(c));
        ...
        InputTextField accountNrInputTextField = FormBuilder.addInputTextField(gridPane, ++gridRow, Res.get("payment.kaspi.accountId"));
        accountNrInputTextField.setValidator(kaspiValidator);
    ...
    public void addFormForEditAccount() {
        ...
        addCompactTopLabelTextField(gridPane, ++gridRow, Res.get("payment.kaspi.accountId"), account.getAccountNr())
                .second.setMouseTransparent(false);
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                ...
                && kaspiValidator.validate(account.getAccountNr()).isValid);
    }
}
