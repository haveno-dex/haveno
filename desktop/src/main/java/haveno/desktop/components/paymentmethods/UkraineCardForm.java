...
public class UkraineCardForm extends PaymentMethodForm {
    private final UkraineCardAccount account;
    private final UkraineCardValidator ukraineCardValidator = new UkraineCardValidator();

    public static int addFormForBuyer(GridPane gridPane, int gridRow,
                                      PaymentAccountPayload paymentAccountPayload) {
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((UkraineCardAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.cardNr"),
                ((UkraineCardAccountPayload) paymentAccountPayload).getAccountNr());
        return gridRow;
    }

    public UkraineCardForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                   InputValidator inputValidator, GridPane gridPane,
                   int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (UkraineCardAccount) paymentAccount;
    }

    @Override
    public void addFormForAddAccount() {
        // this payment method is only for Ukraine/UAH
        account.setSingleTradeCurrency(account.getSupportedCurrencies().get(0));
        CountryUtil.findCountryByCode("UA").ifPresent(c -> account.setCountry(c));
        ...
        InputTextField accountNrInputTextField = FormBuilder.addInputTextField(gridPane, ++gridRow, Res.get("payment.cardNr"));
        accountNrInputTextField.setValidator(ukraineCardValidator);
    ...
    public void addFormForEditAccount() {
        ...
        addCompactTopLabelTextField(gridPane, ++gridRow, Res.get("payment.cardNr"), account.getAccountNr())
                .second.setMouseTransparent(false);
    ...
    public void updateAllInputsValid() {
        allInputsValid.set(isAccountNameValid()
                ...
                && ukraineCardValidator.validate(account.getAccountNr()).isValid);
    }
}
