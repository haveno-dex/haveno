...
public class VippsMobilePayForm extends PaymentMethodForm {
    private final VippsMobilePayAccount account;
    ...
    public static int addFormForBuyer(GridPane gridPane, int gridRow,
        ...
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, ++gridRow, 0, Res.get("payment.account.owner.fullname"),
                ((VippsMobilePayAccountPayload) paymentAccountPayload).getHolderName());
        addCompactTopLabelTextFieldWithCopyIcon(gridPane, gridRow, 1, Res.get("payment.mobile"),
                ((VippsMobilePayAccountPayload) paymentAccountPayload).getMobileNr());
        return gridRow;
    }

    public VippsMobilePayForm(PaymentAccount paymentAccount, AccountAgeWitnessService accountAgeWitnessService,
                              InputValidator inputValidator, GridPane gridPane,
                              int gridRow, CoinFormatter formatter) {
        super(paymentAccount, accountAgeWitnessService, inputValidator, gridPane, gridRow, formatter);
        this.account = (VippsMobilePayAccount) paymentAccount;
    }
    ...
    public void addFormForAddAccount() {
        ...
        InputTextField mobileNrField = FormBuilder.addInputTextField(gridPane, ++gridRow, Res.get("payment.mobile"));
        mobileNrField.setValidator(inputValidator);
        mobileNrField.textProperty().addListener((ov, oldValue, newValue) -> {
            account.setMobileNr(newValue.trim());
            updateFromInputs();
        });

        // the country determines the local Vipps MobilePay currency
