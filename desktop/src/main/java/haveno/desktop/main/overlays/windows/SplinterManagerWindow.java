...
@Slf4j
@Singleton
public class SplinterManagerWindow extends Overlay<SplinterManagerWindow> {
    ...
    private TextArea pubKeyListEntriesTextArea;
    private TextArea seedNodeListEntriesTextArea;
    private TextArea makerFeeEntryTextArea;
    private TextArea takerFeeEntryTextArea;
    ...
    private String seedNodeList = "";
    private String makerFee = "";
    private String takerFee = "";
    private DefaultSeedNodeRepository defaultSeedNodeRepository;
    ...
    //private User user;
    @Inject
    public SplinterManagerWindow(Preferences preferences,
                                    DefaultSeedNodeRepository defaultSeedNodeRepository, PeerManager peerManager, PeerExchangeManager PeerExchangeManager, RequestDataManager requestDataManager, ArbitratorManager arbitratorManager, User user) {
        this.preferences = preferences;
        this.defaultSeedNodeRepository = defaultSeedNodeRepository;
    ...
    public void show() {
        if (!isDisplayed) {
            if (headLine == null)
                headLine = Res.get("wait this actually works?");
    ...
    protected void addThings() {
        closeButtonText(Res.get("shutdown text"));
        onClose(HavenoApp.getShutDownHandler());
        actionButtonText(Res.get("launch haveno text"));

        /*closeButton = new AutoTooltipButton(closeButtonText == null ? Res.get("shutdown text") : closeButtonText);
        closeButton.setOnAction(event -> HavenoApp.getShutDownHandler());

        if (actionHandlerOptional.isPresent()) {
            actionButton = new AutoTooltipButton(Res.get("launch haveno text"));
            actionButton.setDefaultButton(true);
            //TODO app wide focus
            actionButton.requestFocus();
            //actionButton.setOnAction(event -> loadHavenoSplinter());
            //    onAction(this::loadHavenoSplinter);
    ...
    private void addMontent() {
        ...
        final Tuple2<Label, TextArea> labelTextAreaTuple = addTopLabelTextArea(gridPane, ++rowIndex, Res.get("pub label"), Res.get("pubkey promt"));
        pubKeyListEntriesTextArea = labelTextAreaTuple.second;
        pubKeyListEntriesTextArea.setPrefHeight(60);

        final Tuple2<Label, TextArea> labelTextAreaTuple2 = addTopLabelTextArea(gridPane, ++rowIndex, Res.get("seed label"), Res.get("seed prompt"));
        seedNodeListEntriesTextArea = labelTextAreaTuple2.second;
        seedNodeListEntriesTextArea.setPrefHeight(60);

        final Tuple2<Label, TextArea> labelTextAreaTuple3 = addTopLabelTextArea(gridPane, ++rowIndex, Res.get("maker fee label"), Res.get("maker fee prompt"));
        makerFeeEntryTextArea= labelTextAreaTuple3.second;
        makerFeeEntryTextArea.setPrefHeight(60);

        final Tuple2<Label, TextArea> labelTextAreaTuple4 = addTopLabelTextArea(gridPane, ++rowIndex, Res.get("taker fee label"), Res.get("taker fee prompt"));
        takerFeeEntryTextArea= labelTextAreaTuple4.second;
        takerFeeEntryTextArea.setPrefHeight(60);
        ...
        //             setBridgeAddressesByCustomBridges();
        // });*/
        pubKeyList = String.join(",", preferences.getPubKeyList());
        log.info(pubKeyList);
        pubKeyListEntriesTextArea.setText(pubKeyList != null ? pubKeyList : "" );
        pubKeyListEntriesTextArea.textProperty().addListener((observable, oldValue, newValue) -> {
                    pubKeyList = newValue;
                    log.info(pubKeyList);
                    preferences.setPubKeyList(pubKeyList != null ? Arrays.asList(pubKeyList.split(",")) : null);
                    updatePubKeyList();
        });
        seedNodeList = String.join(",", preferences.getSeedNodeList());
        seedNodeListEntriesTextArea.setText(seedNodeList != null ? seedNodeList : "" );
        seedNodeListEntriesTextArea.textProperty().addListener((observable, oldValue, newValue) -> {
                    seedNodeList = newValue;
                    preferences.setSeedNodeList(seedNodeList != null ? Arrays.asList(seedNodeList.split(",")) : null);
                    updateSeedNodeList();
        });
        makerFee = Float.toString(preferences.getMakerFee());
        makerFeeEntryTextArea.setText(makerFee != null ? makerFee : "0.0015");
        makerFeeEntryTextArea.textProperty().addListener((observable, oldValue, newValue) -> {
                    makerFee = newValue;
                    preferences.setMakerFee(makerFee != null ? Float.parseFloat(makerFee) : null);
        });
        takerFee = Float.toString(preferences.getTakerFee());
        takerFeeEntryTextArea.setText(takerFee != null ? takerFee : "0.0075");
        takerFeeEntryTextArea.textProperty().addListener((observable, oldValue, newValue) -> {
                    takerFee = newValue;
                    preferences.setTakerFee(takerFee != null ? Float.parseFloat(takerFee) : null);
        });

    }
    private void updateSeedNodeList(){
        defaultSeedNodeRepository.reloadFromPrefs();
        peerManager.updateSeedNodes();
        PeerExchangeManager.updateSeedNodes();
        requestDataManager.updateSeedNodes();

    }
    private void updatePubKeyList(){
        arbitratorManager.updatePrefs();
    }
    ...
    /*private void loadHavenoSplinter() {
        //log.info("hello peepes");
        //UserThread.runAfter(this::updateSplinterPreferences, 500, TimeUnit.MILLISECONDS);

        hide();
    }
    private void updateSplinterPreferences() {
        //pubkeys are comma separated
        //pubKeyList = "google.com,fakesite.co,monero.gov";
        preferences.setPubKeyList(pubKeyList != null ? Arrays.asList(pubKeyList.split(",")) : null);
