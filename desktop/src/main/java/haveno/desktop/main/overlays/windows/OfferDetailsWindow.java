...
public class OfferDetailsWindow extends Overlay<OfferDetailsWindow> {
    ...
    private void addContent() {
        ...
        addTitledGroupBg(gridPane, ++rowIndex, rows, Res.get(offer.isPrivateOffer() ? "shared.LockedOffer" : "shared.Offer"));
        ...
        if (offerPassphrase != null)
            addConfirmationLabelTextFieldWithCopyIcon(gridPane, ++rowIndex, Res.get("shared.passphrase"), offerPassphrase);
        ...
        confirmButton.setOnAction(e -> {
            if (GUIUtil.canCreateOrTakeOfferOrShowPopup(user, navigation)) {
                if (!isPlaceOffer && offer.isPrivateOffer()) {
                    new PasswordPopup()
                            .headLine(Res.get("offerbook.takeOffer.enterPassphrase"))
                            .onAction(password -> {
                                if (offer.getPassphraseHash().equals(HavenoUtils.getPassphraseHash(password))) {
                                    offer.setPassphrase(password);
